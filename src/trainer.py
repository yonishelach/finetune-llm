...
import torch
# print(f"rank: {rank}, device: count = {torch.cuda.device_count()}, current = {torch.cuda.current_device()}, name = {torch.cuda.get_device_name(0)}")
import shutil
import tempfile
import zipfile
from abc import ABC
...
def train(
    ...
    tokenizer = tokenizer_class.from_pretrained(pretrained_tokenizer, model_max_length=1024)
    if require_tokenization:
        ...
        tokenized_train = Dataset.from_pandas(dataset.as_df())
        tokenized_test = Dataset.from_pandas(test_set.as_df())
    ...
    if data_collator:
        data_collator = DataCollatorForCompletionOnlyLM(tokenizer, **data_collator_kwargs)
    else:
        data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False, **data_collator_kwargs)
    ...
    model_output_path = train_kwargs.get("output_dir", None)
...
def load_my_model():
    # model = transformers.AutoModelForCausalLM.from_pretrained("/User/mlrun-huggingface-demo/trained-model")
    # tokenizer = transformers.AutoTokenizer.from_pretrained("/User/mlrun-huggingface-demo/trained-model")
    # model = transformers.AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-12b")
    # tokenizer = transformers.AutoTokenizer.from_pretrained("EleutherAI/pythia-12b")
    # generate_text = pipeline(model="databricks/dolly-v2-12b", torch_dtype=torch.bfloat16, trust_remote_code=True, device_map="auto")
    tokenizer = GPT2Tokenizer.from_pretrained("/User/mlrun-huggingface-demo/gpt-2-xl-trained-model")  #also try gpt2, gpt2-large and gpt2-medium, also gpt2-xl
    model = GPT2LMHeadModel.from_pretrained("/User/mlrun-huggingface-demo/gpt-2-xl-trained-model")  #also try gpt2, gpt2-large and gpt2-medium, also gpt2-xl
    prompt = "What are the two main components in mlrun?"  # Replace with your desired prompt
    # res = generate_text(prompt)
    response = generate_response(model, tokenizer, prompt)
    print("Generated response:", response)
    # print(res[0]["generated_text"])
    # Test the chatbot
    prompt = "What are mlrun's DataItem used for?"  # Replace with your desired prompt
    response = generate_response(model, tokenizer, prompt)
    print("Generated response:", response)
